...
from infrastructure.database.repo.news import NewsRepo
from infrastructure.database.repo.url import UrlsRepo
...
class RequestsRepo:
    ...
    @property
    def news(self) -> NewsRepo:
        """
        The User repository sessions are required to manage user operations.
        """
        return NewsRepo(self.session)

    @property
    def urls(self) -> UrlsRepo:
        return UrlsRepo(self.session)
...
if __name__ == "__main__":
    ...
    async def example_usage(config: Config):
        ...
        async with session_pool() as session:
            ...
            user = await repo.news.get_or_create_user(
                user_id=12356,
                full_name="John Doe",
                language="en",
                username="johndoe",
